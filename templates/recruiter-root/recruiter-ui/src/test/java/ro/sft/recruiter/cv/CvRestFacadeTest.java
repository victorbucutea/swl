...
import static ro.sft.recruiter.base.util.ReflectionUtil.injectIntoField;
...
import ro.sft.recruiter.interview.model.ProjectDetails;
...
public class CvRestFacadeTest extends DaoTest {
	...
	public void mappingBackIntoManagedObject_deleteChildOneToMany() throws URISyntaxException, IOException {
		...
		Set<Project> projects = deserializedCv.getExperiences().iterator().next().getProjects();
	...
	private void assertExperiencesSerialized(MockHttpResponse response) throws IOException {
		...
		Set<Project> projects = firstExp.getProjects();
		assertEquals("name", projects.iterator().next().getName());
	}
